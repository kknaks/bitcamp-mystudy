...
public class ProjectDaoImpl implements ProjectDao {
  ...
  public boolean update(Project project) throws Exception {
    try (PreparedStatement stmt = con.prepareStatement(
        "update myapp_projects set title=?, description=?, start_date=?, end_date=? where project_id=?")) {
  ...
  public List<User> getMembers(int projectNo) throws Exception {
    try (PreparedStatement stmt = con.prepareStatement(
        "select pm.user_id, u.name from myapp_project_members pm join myapp_users u on pm.user_id = u.user_id where pm.project_id=?")) {
      stmt.setInt(1, projectNo);
      try (ResultSet rs = stmt.executeQuery()) {
        ...
        while (rs.next()) {
          ...
          member.setNo(rs.getInt("user_id"));
